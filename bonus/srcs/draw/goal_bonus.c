...
int	*select_keeper_tex(t_data *data, t_ray *ray)
{
	...
	else if (ray->hit_dir == GOAL_RIGHT && data->goal->position == GOAL_RIGHT && data->goal->has_catched == false)
		keeper_tex = data->tex[RIGHT_FAIL];
	else if (ray->hit_dir == GOAL_LEFT && data->goal->position == GOAL_LEFT && data->goal->has_catched == true)
		keeper_tex = data->tex[LEFT_CATCH];
	else if (ray->hit_dir == GOAL_RIGHT && data->goal->position == GOAL_RIGHT && data->goal->has_catched == true)
...
int		cub_merge_goal_col(t_data *data, t_ray *ray, double pos, double texture_x)
{
	...
	index = (int)(TEXTURE_SIZE * ((int)pos & (TEXTURE_SIZE - 1))) + (int)texture_x;
