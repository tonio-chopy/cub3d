...
void	cub_compute_sprite_dist(t_data *data, t_vec *dist, char elem,
								t_sprite *sprite)
{
	int		ball_i;
	t_vec	*pos;
	...
	// data->cam->plane = ft_rotate_vector_new(trans,
	// ft_to_rad(-data->cam->angle));
...
void	test_angle_vector_consistency(t_data *data)
{
	...
	calc_x = cos(ft_to_rad(data->cam->angle));
	calc_y = -sin(ft_to_rad(data->cam->angle)); // - car y inversé
...
{
	int		index;
	double	sprite_relative_x;
	double	sprite_relative_y;
	double	transform_x;
	double	transform_y;
	double	inv_det;
	t_vec	dist;
	...
	*code = 1;
	...
	pos->yd += 0.5;
	pos->magnitude = 1;
	// t_vec *translate = cub_init_vec_double(-data->player_pos->xd,
	// -data->player_pos->yd);
	// translation
	...
	// det = data->cam->plane->xd * data->cam->dir->yd
	// - data->cam->plane->yd * data->cam->dir->xd;
	// if (det == 0)
	// 	invdet = 1.0 / 0.0000001;
	// else
	// 	invdet = 1.0 / det;
	// dist.xd = invdet * (data->cam->dir->yd * dist.xd
	// - data->cam->dir->xd * dist.yd);
	// dist.yd = invdet * (data->cam->plane->xd * dist.yd
	// - data->cam->plane->yd * dist.xd);
	// if (dist.yd < 0)
	// 	return ;
	dist.xd = pos->xd - data->player_pos->xd;
	dist.yd = pos->yd - data->player_pos->yd;
	...
	transform_x = inv_det * (data->cam->dir->yd * sprite_relative_x
			- data->cam->dir->xd * sprite_relative_y);
	transform_y = inv_det * (-data->cam->plane->yd * sprite_relative_x
			+ data->cam->plane->xd * sprite_relative_y); // C'est la profondeur
	...
	// dist.xd = cos_angle * temp_x - sin_angle * temp_y;
...
// double expected_dir_y = sin(ft_to_rad(data->cam->angle));
// -sin car Y inversé
...
// printf("Expected dir: (%.2f, %.2f)\n", expected_dir_x, expected_dir_y);
	...
	if (data->cup->screenx < 0 || data->cup->screenx >= WIN_W
		|| data->zbuffer[data->cup->screenx] < data->cup->distance)
		return ;
...
// 		return;
	...
	// if (data->cup->screenx < 0 || data->cup->screenx >= WIN_W)
	// 	return ;
	// data->cup->screenx = (int)((FOV_DEGREES
	...
	// data->cup->screenx = (int)(((double)WIN_W * 0.5)
...
void	cub_compute_ball_size(t_data *data, int *code, char elem)
{
	...
	*code = 1;
	...
	if (elem == E_BALL && data->ball->distance < 0.2)
		return ;
	angle_from_dir = ft_to_deg(acos(ball_dist.xd / data->ball->distance));
	tan_plane = ft_to_deg(tan(angle_from_dir));
	if (angle_from_dir < 0)
		data->ball->screenx = (int)(((double)WIN_W * 0.5) * (((FOV_DEGREES / 2)
						- fabs(angle_from_dir)) / FOV_DEGREES));
	else
		data->ball->screenx = (int)(((double)WIN_W * 0.5) * (((FOV_DEGREES / 2)
						+ fabs(angle_from_dir)) / FOV_DEGREES));
	...
	if (relative_pos.yd <= 0)
		return ;
	// data->ball->screenx = (int)(((double)WIN_W * 0.5)
	// * (1.0 + relative_pos.xd / relative_pos.yd));
