...
MSP_INVALID_COLOR);
	if (!ft_strncmp(trimmed, "F", 1))
	{
		if (data->parsed_map->has_ceiling)
			cub_handle_fatal_parse(data, data->parsed_map->fd, line, "Duplicate floor color");
	...
	else
	{
		if (data->parsed_map->has_floor)
			cub_handle_fatal_parse(data, data->parsed_map->fd, line, "Duplicate ceiling color");
...
char *trimmed)
{
	...
	if (data->parsed_map->paths[i])
		cub_handle_fatal_parse(data, data->parsed_map->fd, line, "Duplicate texture path");
	path_start = trimmed + 2;
	while (*path_start == ' ' || *path_start == '\t')
		path_start++;
	if (!*path_start)
		cub_handle_fatal_parse(data, data->parsed_map->fd, line, "Missing texture path");
	path_end = path_start;
	...
	while (*path_end == ' ' || *path_end == '\t')
		path_end++;
	if (*path_end)
		cub_handle_fatal_parse(data, data->parsed_map->fd, line, "Extra content after texture path");
	path_len = path_end - path_start;
	while (path_len > 0 && (path_start[path_len - 1] == ' ' || path_start[path_len - 1] == '\t'))
		path_len--;
...
void	cub_try_add_texture_paths_and_colors(t_data *data, char *line)
{
	...
	while (i < 4)
	{
		if (!ft_strncmp(trimmed, codes[i], 2) && 
			(trimmed[2] == ' ' || trimmed[2] == '\t'))
		{
			has_matched = true;
	...
	if ((!ft_strncmp(trimmed, "F", 1) && (trimmed[1] == ' ' || trimmed[1] == '\t')) ||
		(!ft_strncmp(trimmed, "C", 1) && (trimmed[1] == ' ' || trimmed[1] == '\t')))
